...
{if isset($block_blog_entries) && $block_blog_entries}
	...
	<div class="ia-items blogroll-grid">
			...
			<div class="span6">
				<div class="ia-item-grid">
					{if $one_blog_entry.image}
						...
						<div class="info">
							<h3><a href="{$smarty.const.IA_URL}blog/{$one_blog_entry.id}-{$one_blog_entry.alias}" id="blog_{$one_blog_entry.id}">{$one_blog_entry.title|truncate:25:'...'}</a></h3>
					...
					{else}
						<div class="info">
							<h3><a href="{$smarty.const.IA_URL}blog/{$one_blog_entry.id}-{$one_blog_entry.alias}" id="blog_{$one_blog_entry.id}">{$one_blog_entry.title|truncate:25:'...'}</a></h3>
							<p class="date">{$one_blog_entry.date|date_format:$config.date_format}</p>
							<p Class="body">{$one_blog_entry.body|truncate:400:'...'|strip_tags}</p>
						</div>
					{/if}
				</div>
			</div>
		...
		<div class="panel panel--clean">
			<a href="{$smarty.const.IA_URL}blog/" class="btn btn-primary btn-plain">{lang key='view_all_blog_entries'}</a>
		</div>
	</div>
	{else}
			...
			{foreach $block_blog_entries as $one_blog_entry}
				<div class="media ia-item">
					...
					<div class="media-body">
						...
						<div class="ia-item-body">{$one_blog_entry.body|truncate:$config.blog_max_block:'...'|strip_tags}</div>
					</div>
				</div>
			{/foreach}
		</div>
