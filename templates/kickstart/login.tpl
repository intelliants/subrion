{if !isset($smarty.session['isAllowed'])  }
            ...
            {if $core.providers}
                <div class="social-providers">
                    ...
                    {foreach $core.providers as $name => $provider}
                        <a class="btn btn-block btn-social btn-{$name|lower}"
                           href="{$smarty.const.IA_URL}login/{$name|lower}/"><span
                                    class="fa fa-{$name|lower}"></span> {$name}</a>
                    {/foreach}
                </div>
            {/if}
        </form>
