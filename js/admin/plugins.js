...
var installClick = function (record, action) {
    ...
    if (record.get('notes')) {
        Ext.Msg.show({
            title: _t('invalid_plugin_dependencies'),
            msg: record.get('notes'),
            buttons: Ext.Msg.OK,
    ...
    if (record.get('name') != record.get('file')) {
        Ext.Msg.show({
            title: _t('error'),
            msg: _t('plugin_files_physically_missed'),
            buttons: Ext.Msg.OK,
            icon: Ext.Msg.WARNING
        });
...
Ext.onReady(function () {
        ...
        switch ($(this).attr('href')) {
            ...
            case '#tab-available':
                if (!intelli.available.config) {
                    ...
                    intelli.available.toolbar = new Ext.Toolbar({
                        items: [
                            {
                                xtype: 'textfield',
                                id: 'availableFilter',
                                ...
                                store: new Ext.data.SimpleStore({
                                    fields: ['value', 'title'],
                                    data: [['local', _t('local')], ['remote', _t('remote')]]
                                }),
                                value: 'local',
                                displayField: 'title',
                                valueField: 'value',
    ...
    intelli.installed = new IntelliGrid(
        {
            columns: [
                ...
                {
                    name: 'manage',
                    title: _t('manage_entries'),
                    icon: 'th-list',
                    href: intelli.config.admin_url + '/{value}'
                },
                {name: 'info', title: _t('documentation'), icon: 'info', click: helpClick},
    ...
    intelli.installed.toolbar = new Ext.Toolbar({
        items: [
            {
                xtype: 'textfield',
                id: 'installedFilter',
