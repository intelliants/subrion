	...
	$.jstree.plugin("themes", {
		...
		_fn : {
			set_theme : function (theme_name, theme_url) {
				...
				if($.jstree._themes === false) { check_themes(); }
	...
	$.jstree.plugin("checkbox", {
		...
		_fn : {
			_prepare_checkboxes : function (obj) {
				var type = !obj || obj == -1 ? 'cont' : 'node';
				obj = type != 'node' ? this.get_container() : this._get_node(obj);
				var c, _this = this, t, i = 0;
				obj.each(function () {
					t = $(this);
					if(t.attr(_this.data.checkbox.attr_type) != _this.data.checkbox.attr_value || !_this.data.checkbox.attr_check){
						if(!_this.data.checkbox.child_check)c = t.is("li") && t.hasClass("jstree-checked") ? "jstree-checked" : "jstree-unchecked"; 
						else c = "jstree-unchecked";
						t.find("a:first").not(":has(.jstree-checkbox)").prepend("<ins class='jstree-checkbox'>&#160;</ins>").parent().not(".jstree-checked, .jstree-unchecked").addClass(c);
						i++;
					}
				});
				if(i > 0)
			...
			change_state : function (obj, state) {
				obj = this._get_node(obj);
				if(obj.attr(this.data.checkbox.attr_type) == this.data.checkbox.attr_value && this.data.checkbox.attr_check) return true;
				state = (state === false || state === true) ? state : obj.hasClass("jstree-checked");
				var obj_with_child = this.data.child_check ? obj.find("li").andSelf() : obj;

				if(state) { obj_with_child.removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked"); }
				else {
					obj_with_child.removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked"); 
					if(this.data.ui) { this.data.ui.last_selected = obj; }
					this.data.checkbox.last_selected = obj;
				}
				if(this.data.checkbox.child_check)
				{
				obj.parentsUntil(".jstree", "li").each(function () {
					var $this = $(this);
					if(state) {
